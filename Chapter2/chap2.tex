...
{\large 
	...
	{\large 
			...
			To provide an impression, Figure 2 shows a code excerpt in which the Java API was implicitly used by androidx.  To keep this chapter manageable, it can be simplified  said that androidx is a superset of the Java API Framework. \cite{androidX}
			...
			To beginn its elementary to import the Painter class aswell as the painterResource class (line 13/15).  Then at line 26 a Painter object is created, aswell as definded by the painterResource function. The painterResource function gets an id as an argument, to query the file from the res/drawable folder. The res folder contains all additional app resources (files, images, etc.). Finally the Image can be drawn, by passing the Painter object to an Image function.
			
	}
\subsection{BLE }
{\large
	
	Thus the hardware is a wearable, it is important to manage energy consumption. BLE is designed to minimize energy usage, which is crucial for wearable devices that require extended battery life. A study demonstrated that low-power wearable sensors equipped with BLE could effectively acquire electrophysiological signals like ECG, EMG, and EEG without significant battery drain. \cite{BLEenergy}
...
\subsubsection{BLE Stack}
	...
	\begin{center}
		...
		\label{fig:BLEStack}
		\captionof{figure}{BLE protocol stack (Source: https://de.mathworks.com/help/bluetooth/gs/bluetooth-protocol-stack.html)}
	\end{center}
	...
	\begin{center}
		...
		\captionof{figure}{Attribute Structure (Source: https://de.mathworks.com/help/bluetooth/gs/bluetooth-protocol-stack.html)}
	\end{center}
