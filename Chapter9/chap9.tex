...
{\large 
	...
	And packet reconstruction via PacketReassembler.reassemble.
	
	\underline{\textbf{4. Validation (ResponseChecker)}}\mbox{}\\
	\\

	
	
	
	 Function: check(byteArray: ByteArray)
	This function verifies whether a given byte array corresponds to an expected response from a Bluetooth Low Energy (BLE) communication. The verification consists of multiple steps:
	...
	Checking if the application is in live mode or in 24-Hour mode by calling fetchCurrentMode(). Also feteching all recorded ECG's from the microcontroller ListEKGS().}
		\begin{lstlisting}[label={fig:simpleSourceGen.cs}, caption={Connecting functionality code}]
		
		fun check(byteArray: ByteArray) {
			//First check if its a response packet
			if(byteArray.size <= 2) {
			...
			if(isResponsePacket(byteArray).not()){
				// Not response packet, so we dont care
				return
			}
			
			//Its a response packet, now check for the expected opcode
...
\end{lstlisting}
	...
	To ensure a clear visualization, the function checks whether there is a stored point from the latest ECG reading. If a point exists, a transparent rectangle is drawn at its position, keeping the background clean for the rendering process.
